...
int addTcpClientListRec(modbus_t *ctx )
{
  ...
  PR_DEBUG("newSocket->context=%08x",newSocket->context);
  ...
  {
        PR_DEBUG("ErrorNo %d,%s",errno,strerror(errno));
        PR_DEBUG("ERROR on accept");
      free(newSocket);
      return -1;
  }
...
{  
 ...
 // PR_DEBUG("socketSeverGetNumClients++\n", recordCnt);
  ...
  while ( srchRec )
  {  
 //   PR_DEBUG("socketSeverGetNumClients: recordCnt=%d\n", recordCnt);
    srchRec = srchRec->next;  
//	PR_DEBUG("socketSeverGetNumClients: \n");
    recordCnt++;      
  }
  
//  PR_DEBUG("socketSeverGetNumClients %d\n", recordCnt);
  return (recordCnt);
}
