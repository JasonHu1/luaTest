...
static JSON_Value *json_object_nget_value(const JSON_Object *object, const char *name, size_t n) {
    ...
    int c = json_object_get_count(object);
    for (i = 0; i < c; i++) {
...
void json_replace_string( JSON_Value* value,const char* str)
{
    if(str==NULL)
    {
        return;
    ...
    if(json_value_get_type(value)==JSONString)
    {
        ...
        if (!is_valid_utf8(str, string_len)) {
            return;
        }
        copy = parson_strndup(str, string_len);
        if (copy == NULL) {
            return;
        }
