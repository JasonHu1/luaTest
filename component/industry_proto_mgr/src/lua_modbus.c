...
int __modbus_read_bits(lua_State* L){
    int ret;
    int n = lua_gettop(L);
     PR_DEBUG("stack param count=%d",n);
    if(!lua_istable(L,-1)){
        PR_ERR("stack top is not table");
        return -1;
    }
    lua_pushstring(L,"fc");
    lua_gettable(L,-2);
    ...
    PR_DEBUG("fc=%d,addr=%d,cnt=%d,slave=%d",fc,addr,cnt,slave);
    COMM_INFO_T*conn = user_get_conn_context_byChannel(channel);
    if(conn == NULL){
        PR_ERR("comm context is null");
        return 0;
    }
    ...
    if(cnt>32){
        PR_WARN("static apply memery max is 32 Bytes,BUT cnt=%d",cnt);
        return 0;
    }
    //01 (0x01) Read Coils
    modbus_set_slave(conn->pConnCxt, slave);
    if(ret = modbus_read_bits(conn->pConnCxt,addr, cnt,rdData)!= -1){
         PR_DEBUG("modbus_read_bits ok1");
        for(int i=0;i<cnt;i++){
            printf("%02x ",rdData[i]);
        }
        printf("\r\n");
    }else{
        PR_ERR("errno=%d,faile=%s",errno,modbus_strerror(errno));
    }
...
int __modbus_read_input_bits(lua_State* L){
    ...
    PR_DEBUG("fc=%d,addr=%d,cnt=%d",fc,addr,cnt)    ;
    COMM_INFO_T*conn = user_get_conn_context_byChannel(channel);
    if(conn == NULL){
        PR_ERR("comm context is null");
        return 0;
...
int __modbus_read_registers(lua_State* L){
    ...
    PR_DEBUG("fc=%d,addr=%d,cnt=%d,slave=%d,channel=%d",fc,addr,cnt,slave,channel);

    uint16_t rdData[32]={0};
...
int __modbus_write_register(lua_State* L){
    ...
    int n = lua_gettop(L);
     PR_DEBUG("stack param count=%d",n);
    if(!lua_istable(L,-2)){
        PR_ERR("stack second is not table");
        return 0;
    }
...
int send_report(lua_State* L){
    ...
    PR_DEBUG("dp_cnt=%d,devid=%s,dp_idx=%d",dp_cnt,devid,dp_idx);
    if(dp_cnt<=0){
        lua_pushinteger(L,-1);
        return 1;
    }
    if(LUA_TTABLE!=lua_type(L, -3)){
        PR_ERR("stack -2 NOT TABLE !!! type=%s\r\n",lua_typename(L, lua_type(L, -3)));
        lua_pushinteger(L,-1);
        return 1;
    }
    ...
    for(int i=0;i<dp_cnt;i++){
        ...
        lua_pop(L, 1);
        PR_DEBUG("dp_send[%d].dpid=%d",i,dp_send[i].dpid);

        lua_pushstring(L,"type");
        lua_gettable(L, -2);
        dp_send[i].type=lua_tointeger(L,-1);
        lua_pop(L, 1);
        PR_DEBUG("dp_send[%d].type=%d",i,dp_send[i].type);
        ...
        switch(dp_send[i].type){
            case PROP_BOOL:
                ...
                PR_DEBUG("dp_send[%d].value.dp_bool=%d",i,dp_send[i].value.dp_bool);
                break;
            case PROP_VALUE:
                dp_send[i].value.dp_value=lua_tointeger(L,-1);
                PR_DEBUG("dp_send[%d].value.dp_value=%d",i,dp_send[i].value.dp_value);
                break;
            case PROP_STR:
                dp_send[i].value.dp_str=(char*)lua_tostring(L,-1);
                PR_DEBUG("dp_send[%d].value.dp_str=%s",i,dp_send[i].value.dp_str);
                break;
            case PROP_ENUM:
                dp_send[i].value.dp_enum=lua_tointeger(L,-1);
                PR_DEBUG("dp_send[%d].value.dp_enum=%d",i,dp_send[i].value.dp_enum);
                break;
            case PROP_BITMAP:
                dp_send[i].value.dp_bitmap=lua_tointeger(L,-1);
                PR_DEBUG("dp_send[%d].value.dp_bitmap=%d",i,dp_send[i].value.dp_bitmap);
                break;
            default:
                break;
        }
...
int app_modbus_read_registers(char*devid,int addr,int dpid,int type,int slave,int channel){
    int ret;
    TY_OBJ_DP_S dp_send;
    PR_DEBUG("addr=%d,slave=%d,channel=%d",addr,slave,channel);
    ...
    //01 (0x01) Read Coils
    modbus_set_slave(conn->pConnCxt, slave);
    if(ret = modbus_read_registers(conn->pConnCxt,addr, 1,rdData)!= -1){
         switch(dp_send.type){
            case PROP_BOOL:
                dp_send.value.dp_bool=(BOOL_T)rdData[0];
                PR_DEBUG("dp_send.value.dp_bool=%d",dp_send.value.dp_bool);
                break;
            case PROP_VALUE:
                dp_send.value.dp_value=(INT_T)rdData[0];
                PR_DEBUG("dp_send.value.dp_value=%d",dp_send.value.dp_value);
                break;
            case PROP_STR:
                //dp_send.value.dp_str=(char*)rdData;
                PR_DEBUG("dp_send.value.dp_str=%s",dp_send.value.dp_str);
                break;
            case PROP_ENUM:
                dp_send.value.dp_enum=(UINT_T)rdData[0];
                PR_DEBUG("dp_send.value.dp_enum=%d",dp_send.value.dp_enum);
                break;
            case PROP_BITMAP:
                dp_send.value.dp_bitmap=(UINT_T)rdData[0];
                PR_DEBUG("dp_send.value.dp_bitmap=%d",dp_send.value.dp_bitmap);
                break;
            default:
                break;
        }
        if((ret = dev_report_dp_json_async(devid,&dp_send,1))!=OPRT_OK){
            PR_ERR("ret=0x%04x",ret);
        }
    }else{
        PR_ERR("errno=%d,faile=%s",errno,modbus_strerror(errno));
