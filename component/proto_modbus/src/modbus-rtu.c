...
static int _modbus_set_slave(modbus_t *ctx, int slave)
{
    ...
    if (slave >= 0 && slave <= 247) {
        ctx->slave = slave;
        printf("< JasonDbg >[%s:%d]%s() ctx->slave=%d\r\n",__FILE__,__LINE__,__FUNCTION__,ctx->slave);
    } else {
        errno = EINVAL;
        return -1;
    }
...
#endif
        printf("[JasonDbg:%d]%s() ctx->s=%d\r\n",__LINE__,__FUNCTION__,ctx->s);
                              ...
                              struct timeval *tv, int length_to_read)
{
    ...
    while ((s_rc = select(ctx->s+1, rset, NULL, NULL, tv)) == -1) {
        printf("< JasonDbg >[%s:%d]%s() s_rc=%d\r\n",__FILE__,__LINE__,__FUNCTION__,s_rc);
        if (errno == EINTR) {
            if (ctx->debug) {
